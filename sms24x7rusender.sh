...
sendresult=`curl -X POST --silent -d "method=push_msg&format=json&api_v=1.1&email=$login&password=$password&text=$message&phones=$phones" https://api.sms24x7.ru/ 2>&1 | grep -Eo '(?<="err_code":)(\d)+'`
